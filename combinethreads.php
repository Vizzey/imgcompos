<?php
	...
	else
	{
     ...
     //  imagealphablending($im[3],true);
       ...
       class multimages extends Thread
       {
            public function __construct($i,$j,$im,$width,$height) 
            {
                $this->i = $i;
                $this->j = $j;
                $this->im = $im;
            ...
            public function run() 
            {
                lightenarea($this->i,$this->j,$this->im,$this->width,$this->height);
            }
       }
	 ...
	 for($j=1;$j<$sizey-$areasize;$j+=$areasize)
	 {
	   ...
	   //lightenarea($i,$j,$im,$areasize,$areasize);
             $thr[]= new multimages($i,$j,$im,$areasize,$areasize);
	 }
	 foreach ($thr as $t)
             $t->start();
         foreach ($thr as $t)
